...
export const ChartAreaInteractiveSkeleton = () => {
  return (
    <div className="flex-1 pt-0">
      <div className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
        <div className="grid flex-1 gap-1">
        ...
        <Skeleton className="hidden h-10 w-[160px] rounded-lg sm:ml-auto sm:flex" />
      </div>
        ...
        <div className="relative aspect-auto h-[250px] w-full">
          ...
          <div className="absolute inset-0 flex items-end justify-between px-4 pb-8">
            {Array.from({ length: 6 }).map((_, index) => (
              <Skeleton key={`chart-bar-${index}`} className="h-4 w-8" />
            ))}
          </div>
        </div>
