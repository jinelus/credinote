...
export const OrderSelect = () => {
	...
	const handleValueChange = (value: string) => {
		const selectedElement = selectElements.find(
			(element) => `${element.orderBy}-${element.order}` === value,
		)
		if (selectedElement) {
			handleQuery(
				selectedElement.orderBy,
				selectedElement.order as 'asc' | 'desc',
			)
		}
	}
			...
			>
				{selectElements.map((element, index) => (
					<SelectItem
						...
						className="truncate px-2 text-center text-sm hover:bg-slate-50 hover:outline-none"
					>
						{element.label}
					</SelectItem>
				))}
