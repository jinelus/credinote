...
export const handleCpfInputFormatting = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  if (value.length > 9) {
    formattedValue = `${formattedValue.substring(0, 11)}-${value.substring(9, 11)}`
  }
  if (value.length > 11) {
...
export const handleTelephoneInput = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  if (value.length > 2) {
    formattedValue = `${formattedValue.substring(0, 3)}) ${value.substring(2)}`
  }
  if (value.length > 7) {
    formattedValue = `${formattedValue.substring(0, 10)}-${value.substring(7)}`
  }
